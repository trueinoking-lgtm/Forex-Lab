"""Bank of England adapter for SONIA."""
...
class BankOfEnglandAdapter(BaseSourceAdapter):
    def identify_source(self) -> str: return "bank_of_england"
    def build_request(self, source, start_date, end_date):
        for f in ["institution","official_source_location","exact_series_identifier","first_available_date","latest_available_date"]:
            if not getattr(source, f, None): raise SourceAcquisitionError(f"BoE registry record missing: {f}")
    ...
    def acquire_raw(self, request): raise SourceAcquisitionError("BoE acquisition not yet implemented.")
