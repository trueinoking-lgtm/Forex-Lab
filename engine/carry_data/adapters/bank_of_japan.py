"""Bank of Japan adapter for TONA."""
...
class BankOfJapanAdapter(BaseSourceAdapter):
    def identify_source(self) -> str: return "bank_of_japan"
    def build_request(self, source, start_date, end_date):
        for f in ["institution","official_source_location","exact_series_identifier","first_available_date","latest_available_date"]:
            if not getattr(source, f, None): raise SourceAcquisitionError(f"BOJ registry record missing: {f}")
    ...
    def acquire_raw(self, request): raise SourceAcquisitionError("BOJ acquisition not yet implemented.")
